...
/**
 * @name AbstractIntegerListIterator @date 2012.09.21
 *
...
public abstract class AbstractNCIntegerListIterator {
    ...
    public AbstractNCIntegerListIterator(Integer numberOfCounters, Integer stepSize) {
     ...
     * config is iterated on its possible orders using the combinatoricsLib. This implementation relies on a new version of 
     * combinatoricsLib which does not produced repeats anymore.
     *
     * This method computes the next element to be given, so calling it twice without a call to next() will lose one
     * generated element.
     *
        ...
        if (hasNext && (swapIterator == null || !swapIterator.hasNext())) {
            ...
            while (moveTokensBetweenSlot) {
                if (counters.get(indexToDecrease) - stepSize >= this.minPerSlot) {
                    ...
                    /**
                     * The exit condition is given by checking that all positions to the left of the index to decrease
                     * are at the minimum and that the indexToIncrease is at the maximum position.
                     */
